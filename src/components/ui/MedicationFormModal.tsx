    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      ...
      >
        <div className="p-6 space-y-4">
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">Medicine Name</label>
              <input type="text" value={name} onChange={e => { setError(null); setName(e.target.value) }}
                placeholder="e.g. Metformin"
                className="w-full h-11 px-4 bg-slate-50 border border-slate-200 text-on-surface rounded-xl focus:border-primary outline-none text-sm font-semibold" />
            </div>

            <div className="space-y-1">
              <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">Strength</label>
              <div className="flex gap-2">
                <input type="number" value={strengthNum} onChange={e => { setError(null); setStrengthNum(e.target.value) }}
                  ...
                  className="flex-1 h-11 px-4 bg-slate-50 border border-slate-200 text-on-surface rounded-xl focus:border-primary outline-none text-sm font-semibold" />
                <select value={strengthUnit} onChange={e => setStrengthUnit(e.target.value)}
                  className="w-20 h-11 px-3 bg-slate-50 border border-slate-200 text-on-surface rounded-xl focus:border-primary outline-none text-sm font-semibold appearance-none cursor-pointer">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">Timing</label>
        ...
        <div className="flex gap-3 px-6 pb-6">
          <button type="button" onClick={onClose}
            className="flex-1 h-11 text-sm font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer">
            Cancel
          </button>
